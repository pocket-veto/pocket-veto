...
dependencies {
    ...
    // JVM unit tests (Protocol-mirror + FrameCodec). AGP's `test` task runs
    // JUnit 4 natively, so no `useJUnitPlatform()` / `testOptions` changes are
    // needed. kotlinx-serialization-json is already an `implementation` dep
    // above, so it is visible to test sources without re-adding it here.
